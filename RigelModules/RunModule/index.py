...
class RunModule:
    ...
    def exec(self, *args):
        ...
        for each_command in to_run:
            try:
                ...
                p = subprocess.Popen(each_command, stdout=subprocess.PIPE)
    ...
    def runAndWait(self, args, callback=None):
        try:
            p = subprocess.Popen(" ".join(args), stdout=subprocess.PIPE)
            p.wait()
    ...
    def runAndWaitWithOutput(self, args, callback=None, stdout=True, stderr=False):
        ...
        try:
            ...
            p = subprocess.Popen(" ".join(args), stdout=subprocess.PIPE if stdout else None, stderr=subprocess.PIPE if stderr else None)
    ...
    def getInstance(cls, env):
        if cls.__module_instance is None:
            cls.__module_instance = RunModule(env)
        return cls.__module_instance


def getModuleInstance(env):
    return RunModule.getInstance(env)
